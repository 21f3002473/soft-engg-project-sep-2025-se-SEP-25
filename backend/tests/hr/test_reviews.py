...
def test_review_create_unauthorized(base_url):
    ...
    r = httpx.post(f"{base_url}/hr/review", json=payload)
...
def test_review_detail_success(base_url, auth_hr):
    ...
    r = httpx.get(f"{base_url}/hr/review/{rev_id}", headers=auth_hr)
...
def test_review_detail_not_found(base_url, auth_hr):
    r = httpx.get(f"{base_url}/hr/review/999999", headers=auth_hr)
...
def test_review_detail_unauthorized(base_url):
    r = httpx.get(f"{base_url}/hr/review/1")
    assert r.status_code in [401, 403]
...
def test_review_delete_success(base_url, auth_hr):
    ...
    r = httpx.delete(f"{base_url}/hr/review/{rev_id}", headers=auth_hr)
...
def test_review_delete_not_found(base_url, auth_hr):
    r = httpx.delete(f"{base_url}/hr/review/999999", headers=auth_hr)
