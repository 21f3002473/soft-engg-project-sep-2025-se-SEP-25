...
class AdminAddEmployeeValidator(BaseModel):
	name: str = Field(min_length=1)
	role: str = Field(description="Role name such as 'HR', 'Product Manager', or 'Employee'")
...
class AdminBackupResource(Resource):
	...
	def put(
		...
		return {"message": "Backup configuration updated", "count": len(payload.backups)}
...
class AdminAccountResource(Resource):
	...
	def put(
		...
		if payload.name and payload.name.strip() and payload.name.strip() != current_user.name:
