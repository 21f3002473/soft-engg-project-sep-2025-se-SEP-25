...
def register_admin(
    ...
    return AdminRegistrationResource().post(payload, session)
...
def admin_summary(
    ...
    return AdminDashboardResource().get(current_user, _root, session)
...
def list_employees(
    ...
    return AdminEmployeeResource().get(current_user, _root, session)
...
def add_employee(
    ...
    return AdminEmployeeResource().post(payload, current_user, _root, session)


@admin_router.get("/backup-config")
def get_backup_config(
    current_user: User = Depends(get_current_active_user),
    _root: User = Depends(require_root()),
    session: Session = Depends(get_session),
):
    return AdminBackupResource().get(current_user, _root, session)


@admin_router.put("/backup-config")
def put_backup_config(
    ...
    return AdminBackupResource().put(payload, current_user, _root, session)
...
def get_logs(
    ...
    offset: int = Query(0, ge=0),
    current_user: User = Depends(get_current_active_user),
    _allowed: User = Depends(can_view_system_logs()),
    session: Session = Depends(get_session),
):
    return AdminLogsResource().get(limit, offset, current_user, _allowed, session)
...
def get_updates(
    ...
    return AdminUpdatesResource().get(current_user, _root)
...
def get_account(
    ...
    return AdminAccountResource().get(current_user, _root)


@admin_router.put("/account")
def update_account(
    payload: AccountUpdatePayload,
    current_user: User = Depends(get_current_active_user),
    _root: User = Depends(require_root()),
    ...
    return AdminAccountResource().put(payload, current_user, _root, session)
