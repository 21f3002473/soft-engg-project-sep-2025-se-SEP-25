...
from app.database.product_manager_models import Requirement, Project, Client, StatusTypeEnum
...
class PMRequirementsAgent:
    ...
    def __init__(self, session: Session):
        ...
        # Initialize Groq LLM with currently supported model
        self.llm = ChatGroq(
    ...
    def analyze(
        self, 
        project_id: int, 
        new_requirement_id: Optional[int] = None
    ) -> Dict:
        """
    ...
    def _fetch_data(self, state: RequirementAnalysisState) -> RequirementAnalysisState:
        ...
        try:
            ...
            logger.info(f"Fetched {len(requirements)} requirements for project {project_id}")
            return state
    ...
    def _analyze_requirements(self, state: RequirementAnalysisState) -> RequirementAnalysisState:
        """Use AI to analyze requirement patterns and provide insights."""
        try:
            requirements = state["requirements"]
            project = state["project_data"]
            
            # Calculate statistics FIRST (before AI call)
            total = len(requirements)
            pending = sum(1 for r in requirements if r["status"] == StatusTypeEnum.PENDING)
            in_progress = sum(1 for r in requirements if r["status"] == StatusTypeEnum.IN_PROGRESS)
            completed = sum(1 for r in requirements if r["status"] == StatusTypeEnum.COMPLETED)
            completion_rate = (completed / total * 100) if total > 0 else 0
            
            # Initialize analysis with statistics (ensures it always exists)
            state["analysis"] = {
                "statistics": {
            ...
            try:
                ...
                prompt = ChatPromptTemplate.from_messages([
                    SystemMessage(content="""You are an expert project manager and requirements analyst. 
...
""")
                ...
                logger.info(f"AI analysis completed for project {project['project_id']}")
    ...
    def _generate_fallback_analysis(self, project: Dict, requirements: List[Dict], stats: Dict) -> str:
...
"""
    ...
    def _generate_execution_plan(self, state: RequirementAnalysisState) -> RequirementAnalysisState:
        ...
        try:
            ...
            # Prepare requirements by status
            pending_reqs = [r for r in requirements if r["status"] == StatusTypeEnum.PENDING]
            in_progress_reqs = [r for r in requirements if r["status"] == StatusTypeEnum.IN_PROGRESS]
            completed_reqs = [r for r in requirements if r["status"] == StatusTypeEnum.COMPLETED]
            
            # Generate basic execution plan first
            execution_plan = []
            ...
            try:
                prompt = ChatPromptTemplate.from_messages([
                    SystemMessage(content="""You are an expert project execution planner.
...
""")
                ...
                logger.error(f"AI execution plan failed: {str(ai_error)}, using basic plan")
                state["execution_plan_ai"] = "Execution plan generated using rule-based approach."
    ...
    def _create_recommendations(self, state: RequirementAnalysisState) -> RequirementAnalysisState:
        ...
        try:
            ...
            if analysis['statistics']['in_progress'] > 3:
                recommendations.append("⚠️ HIGH WORKLOAD: Focus on completing in-progress tasks before starting new ones.")
            
            if analysis['statistics']['completion_rate'] > 70:
                recommendations.append("✅ EXCELLENT PROGRESS: Project is on track. Maintain current velocity.")
            elif analysis['statistics']['completion_rate'] < 30:
                recommendations.append("📊 LOW COMPLETION: Review timeline and resource allocation.")
            
            if state["new_requirement_id"]:
                recommendations.append("🆕 NEW REQUIREMENT: Review impact on timeline and communicate with stakeholders.")
            
            if analysis['statistics']['pending'] > 5:
                recommendations.append("📋 LARGE BACKLOG: Prioritize and break down requirements.")
            
            # Try to get AI recommendations
            try:
                prompt = ChatPromptTemplate.from_messages([
                    SystemMessage(content="""You are a senior project management consultant.
...
""")
                ...
                ai_recommendations = [
                    ...
                    if line.strip() and (line.strip().startswith('-') or line.strip().startswith('•') or (len(line.strip()) > 0 and line.strip()[0].isdigit()))
                ]
                ...
                logger.error(f"AI recommendations failed: {str(ai_error)}, using basic recommendations")
                recommendations.append("💡 TIP: Regular status updates help track progress effectively.")
    ...
    def _compile_report(self, state: RequirementAnalysisState) -> RequirementAnalysisState:
        ...
        try:
            ...
            if state["new_requirement_id"]:
                new_req = next(
                    (r for r in state["requirements"] if r["is_new"]), None
                )
            ...
            state["final_report"] = {
                ...
                "summary": {
                    **state["analysis"]["statistics"],
                    "ai_analysis": state["analysis"].get("ai_analysis", "No analysis available"),
                },
                "new_requirement": {
                    "requirement_id": new_req["requirement_id"] if new_req else None,
                    "description": new_req["description"] if new_req else None,
                } if new_req else None,
