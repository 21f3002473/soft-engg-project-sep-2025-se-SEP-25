...
def get_all_reviews(session: Session) -> List[PerformanceReview]:
    return session.exec(select(PerformanceReview).order_by(PerformanceReview.created_at.desc())).all()


def get_reviews_by_user(user_id: int, session: Session) -> List[PerformanceReview]:
    return session.exec(
        select(PerformanceReview).where(PerformanceReview.user_id == user_id).order_by(PerformanceReview.created_at.desc())
    ).all()
...
def create_review(payload: Dict[str, Any], session: Session) -> PerformanceReview:
    ...
    review = PerformanceReview(user_id=payload["user_id"], comments=payload.get("comments"), rating=payload["rating"])
    session.add(review)
    session.commit()
...
def update_review(review_id: int, payload: Dict[str, Any], session: Session) -> PerformanceReview:
